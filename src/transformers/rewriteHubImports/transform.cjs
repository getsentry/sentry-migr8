...
module.exports = function transform(fileInfo, api, options) {
  if (!hasSentryImportOrRequire(fileInfo.source, SENTRY_HUB_PACKAGE)) {
    return undefined;
  }
  ...
  findHubImports(root, j).forEach(importPath => {
    if (importPath.node.specifiers?.length !== 1 || importPath.node.specifiers[0].type !== 'ImportNamespaceSpecifier') {
      return;
    }
